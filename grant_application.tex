...
We hypothesize that functional metrics correlate better with successful BiV repair than the measured volume and pressure. The pressure, volume, and outflow data yield A) the standard clinical metrics, end-diastolic volume and pressure, B) ventricular work and ventricular and arterial elastance, surrogates of contractility and circulation resistance, and C) myocardial stiffness and contractility from the DT created on-the-fly during the exam. This aim delivers selection criteria built on capability (function), and not only on size.
}
...
\includegraphics[width=0.30\textwidth]{figures/intro/DORV06_4ch.jpg}
    \caption{\textbf{Schematics of a heart with double-outlet right ventricle (DORV) with a borderline-size left ventricle (LV) a ventricular septal defect (VSD), and MR image.
...
Rather than analyzing the iCMR data offline -- of which we performed several proof-of-concept works\cite{Ruijsink_Fontans_PLOSone,gusseva23,gusseva25,gusseva25a} (see a selected example in Figure~\ref{fig:AJP2025}) -- \textbf{we will synthesize the CMR and pressure data and biomechanical models of the patients' cardiovascular physiology ``on-the-fly'' during the iCMR exam.} 
This will provide morphology and functional indicators -- such as ventricular volumes, function of valves, vascular resistance in 
\begin{wrapfigure}{r}{0.45\textwidth}
    \centering
    ...
    \vspace{-2pt}
\end{wrapfigure}
various parts of the circulation 
...
In several works supervised by the PI, it was shown that it is technically feasible for  medical doctors to set up and run such models\cite{ruijsink20,ruijsink_FIMH2019,legall20,legall19,waugh_FIMH2021}. In these projects, the deep insight of medical doctors into the actual physiological and clinical aspects allowed for some key adjustments of the problem-specific modeling framework, and  subsequent invaluable interpretation of the results and dissemination within clinical communities. \textbf{The approach of a direct involvement of medical doctors in the work employing the Digital Twins -- while using modeling frameworks suitable for the clinical environment -- will be crucial for actual successful clinical translations of the novel techniques in the planning of optimal clinical management of the patients with SVHD and a borderline-size ventricle. } 
...
demonstrating predictivity of a 3D  electro-mechanical cardiac model for cardiac resynchronization therapy\cite{sermesant12} (Figure~\ref{fig:CRT})
...
\begin{minipage}{0.65\textwidth} %.66
...
\subfigure[0D-model\cite{caruel13} predictions for a patient with ccTGA undergoing double switch procedure\cite{gusseva25}.]{
    \begin{minipage}{0.28\textwidth} %.33
        %\centering
...
\caption{\textbf{Illustration of models of different complexity in clinical proof-of-concept works.} The substantial acceleration of computation and simplification of Digital Twin creation in (b) 
allowed for simulating a wide number of scenarios -- several levels of pulmonary band tightening and complex double-switch surgery (swapping the outflows and inflows of both ventricles) -- suitable to pursue even for a larger patient cohort (or actual clinical trial). 
    } 
...
As for the actual cohort of the patients with a borderline-size ventricle, the pilot study pursued by our investigative team and presented at the Cardiopulmonary Physiome Workshop 2026\cite{chabiniok_Physiom2026abstract} contains six patients (P1-6) with double-outlet right ventricle (DORV) who underwent iCMR exam pre-BiV conversion. The iCMR data provided information about the volume and pressure loading (EDV and EDP); end-systolic volume (ESV). We adapted the methods from our previous work\cite{gusseva25} to create Digital Twins, which augmented the data analysis by providing the myocardial contractility.  
%Figure~\ref{fig:DT_baseline} illustrates the Digital Twin created from a selected patient. 

The models predicted that P1-2 would require post-BiV conversion an increase of HR by 20-30\%, in addition to the 
...
Within our Heart Center -- and with direct feedback provided by cardiologists and surgeons -- we have also recently advanced the proof-of-concept of automatizing of DT creation and a {\it in silico} interventions using a dedicated Graphical User Interface (GUI). The Digital Twin in Figure~\ref{fig:GUI} is motivated by the closed-loop Fontan circulation we previously modeled\cite{aramburu24}. Using the GUI, the clinicians of our Heart Center had the chance to test common scenarios of pharmacological intervention (e.g., pulmonary vasodilators\cite{ruijsink_FIMH2019}) or the effect of a closure of the Fontan fenestration (an artificial connection which allows for a portion of blood from inferior vena cava bypassing the pulmonary circulation and directly entering the heart through the atrium (as displayed in Figure~\ref{fig:GUI}). 
...
%    \includegraphics[width=1\columnwidth]{figures/schematics/circuit_DORVwithGlenn.pdf}
    ...
    \includegraphics[width=1\columnwidth]{figures/prelim_studies/GUI/GUI_fenestrationClosure.png}           
    \caption{\textbf{
    Illustratio of a GUI prototype for Digital Twin creation from iCMR data and the subsequent \textit{in silico} intervention FFTO (Fontan fenestration test occlusion). 
    }
    } 
    \label{fig:GUI}
    \vspace{-2pt}
...
First, we will develop a method of DT-augmented iCMR analysis for the patients with  with borderline-size LV and double-outlet right ventricle (DORV).  DORV is a complex congenital heart defect in which both
great arteries -- aorta and the pulmonary trunk -- arise entirely or predominantly from RV (we recall Figure~\ref{fig:intro_DORV}). This condition has a pooled global birth prevalence of approximately
...
While all patients with a borderline-size ventricle considered for a BiV repair represent an inhomogeneous group, the group of patients with DORV is relatively homogeneous. Having currently $>$ \textcolor{red}{20} retrospective iCMR datasets of patients with DORV prior to BiV repair (with the clinical outcomes post-surgery) and counting with \textasciitilde{}\textcolor{red}{20}  new patients with DORV in the complex BiV repair yearly, we will use this group for the initial part of the project, prior to extending the focus on other patients with borderline-size LVs or RVs. 
...
We will employ the biomechanical heart model of a reduced geometry and kinematics\cite{caruel13}. This model was previously used in our clinical research projects\cite{ruijsink20,legall20,gusseva23,gusseva25}. Ventricular geometry and kinematics are reduced to a sphere with inner radius $R$ and wall thickness $d$. %(Figure \ref{fig:schematics}). 
...
\begin{enumerate}
\item Ventricular outflow into aorta and main pulmonary artery (MPA), with a pulmonary artery band (put in place to reduce the risk of developing pulmonary hypertension); the pulmonary band is represented as a forward resistance of the pulmonary valve\cite{gusseva25}.
...
\item RV and a borderline-size LV will be represented by two ventricle blocks, which will be connected via a VSD (with the resistance creating the LV-to-RV pressure difference as observed in the pressure data). This  configuration with an increased complexity will be used for smaller-sized VSDs, restricting the LV-to-RV flow. 
 \end{enumerate}
...
The Digital Twin will act as ``biophysical filter'', which will reduce the uncertainty in the iCMR data\cite{gusseva23,gusseva25} and provide additional quantities of clinical interest, not directly visible in the data. Namely, the iCMR data analysis will be augmented by:  ventricular pressure-volume (PV) loops of a high quality\cite{gusseva25} and internal/external work;  ventricular elastance\cite{legall19}; myocardial stiffness and the actual value of contractility and resistances in the circuit\cite{ruijsink20,legall20}.

The automated adjustment of the model to the patient-specific regime will incorporate machine learning methods\cite{skardova2026_Heliyon} (providing an initial guess of the patient-specific parameters) and the Kalman-like filtering\cite{chabiniok12} that we previously used in the connection with a complex cardiac biomechanical model. The recent work\cite{kimmig_FIMH2025} presents the sensitivity analysis of the same cardiac model to input data in the context of ICU monitoring; the method will be adapted to the problem of using iCMR data. 

\textbf{Sub-aim 1b: DT-augmented iCMR platform (Years 1 to 2)}
While the PhysioBlocks library is suitable for prototyping and testing tasks, it would be difficult to upscale it to the routine clinical use (and operate it by a non-modeling experts). Therefore, a customized software will be implemented in parallel by the Twynova med-tech company. 
...
The experience of Twynova with providing solutions for the ICU and general anesthesia setups (continuous waveforms of pressure and cardiac output, real time adapting the predefined Digital Twins\cite{kimmig_FIMH2025}) will allow for a technical solution incorporating the specific data output of iCMR (discrete values characterizing the anatomy of the ventricles, and pressure and flow waveforms). The fast engine (the expected acceleration factor is around 10, in comparison to the Python-based PhysioBlocks prototyping), will be suitable for the full automation of Digital Twin generation during the iCMR procedure.
...
Sub-aim 1a will introduce the novel ingredients for decision making to the communities of pediatric cardiothoracic surgery and cardiology (while exemplifying on the hearts with DORV and a borderline-size LV), and Sub-aim 3 will demonstrate that the method can be generalized to the whole cohort of patients considered for the complex BiV repair (with the potential for any complex intervention planning). The DT-augmented iCMR platform (Sub-aim~2) will be essential in establishing the pipeline for including all newly incoming patients in Years 4-5. %, which will be crucial when incorporating DTs in the planning of complex surgeries.  
The DT-augmentation of multi-modality data analysis will provide a deep (patho-)physiological insights into the the question, why some hearts have the capacity to grow and / or sustain circulation, and why some hearts tend to fail under such loading. Therefore, we expect methodological (Digital Twinning) and technological (automated platform of DT-augmented clinical data analysis), as well as physiological and clinical results.

\textbf{\textcolor{red}{Major Risks / Pitfalls and Alternate Strategies}}
...
%    \includegraphics[width=1\columnwidth]{figures/schematics/circuit_DORVwithGlenn.pdf}
    ...
    \caption{\textbf{{In silico} biventricular repair (left ventricular septation and connection with the systemic circulation) and example predictions of the function of the ventricle post-repair (right).
    }} 
...
Plots in Figure~\ref{fig:BiV_repair} demonstrate such {\it in silico} predictions when varying the LV preload and chronotropic effects in a patient with DORV (for whom the Digital Twin at baseline was depicted in Figure~\ref{fig:DT_baseline}). The results in this particular patient  suggest a more complicated post-surgery course. 
...
\textbf{Hypothesis:} 
\textcolor{red}{
The predicted non-favorable outcome (as defined above) will correlate with some indicators of the immediate clinical course during the post-surgery hospital stay (among which, we will use left atrial pressure in the first 48 hours; heart rate; inotrope requirement; end-organ dysfunction reflective of low cardiac output (BUN/Cr, LFTs, UOP); other hospital complications or re-interventions; ICU Length of Stay; Hospital Length of Stay).
}

\textbf{\textcolor{red}{Expected results:}} 
...
Even if implementing a combination of 0D heart chambers (described above),
there will not be a direct interaction between the two ventricles (via the septum). After the BiV repair, we rather expect the risk of the smaller ventricle failure; the increased loading pressure in the borderline-size ventricle could lead to a volume decrease of the other ventricle (via the septum bulging), which would be rather modest, however. Nevertheless, if such a direct LV-RV interaction via the septum turns out to be of a functional importance, we will consider addressing it, e.g., via the recently reported approach\cite{nguyen_FIMH2025}. 

There is a limited risk in recruiting a sufficient number of patients with DORV within the BiV repair program. However, having already $>$\textcolor{red}{70} datasets available in total and including $>$\textcolor{red}{20} cases with DORV (from the Complex BiV repair program at UT Southwestern), and the current pace of around \textcolor{red}{30} new patients annually, this risk is relatively modest. 
Independently of this project, we aim to collaborate with other centers performing BiV repair -- such as Boston Children’s as in our recent joint works\cite{gusseva23,gusseva25a}; Duke (with active collaboration of  Dr. Andersen (Director of the Complex BiV Repair program at UT Southwestern, Dallas and Co-Investigator of this project) and Dr. Turek (Duke)); and other centers. Creating such a network will be invaluable, particularly in the next step of extending the work to the multi-center study. 
...
\begin{wrapfigure}{r}{0.38\textwidth}
    \centering
    \includegraphics[width=0.38\textwidth]{figures/figures_gr_page2.pdf}
    \caption{\textbf{Feedback loop in our cardiac growth model.}}
    \label{fig_cardiac_gr_loop}
    \vspace{-12pt}
\end{wrapfigure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\clearpage
\section*{\normalsize{REFERENCES CITED \textnormal{(\textbf{Senior Personnel}, \underline{Collaborators})}}}
